...
ResultCode Process::HeapFree(VAddr target, u32 size) {
    ...
    while (interval_target != target + size) {
        ...
        u32 backing_offset = backing_memory - Memory::fcram.data();
...
ResultCode Process::Map(VAddr target, VAddr source, u32 size, VMAPermission perms) {
    ...
    while (interval_source != source + size) {
        ...
        u8* backing_memory =
            source_vma->second.backing_memory + (interval_source - source_vma->second.base);

        auto target_vma = vm_manager.MapBackingMemory(interval_target, backing_memory,
                                                      interval_size, MemoryState::Alias);
