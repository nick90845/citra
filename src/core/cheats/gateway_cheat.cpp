...
template <typename T>
static inline typename std::enable_if_t<std::is_integral_v<T>> WriteOp(
...
template <typename T>
static inline typename std::enable_if_t<std::is_integral_v<T>> CompOp(
    const GatewayCheat::CheatLine& line, State& state, std::function<T(VAddr)> read_func,
    std::function<bool(T)> comp) {
    u32 addr = line.address + state.offset;
...
template <typename T>
static inline typename std::enable_if_t<std::is_integral_v<T>> IncrementiveWriteOp(
    const GatewayCheat::CheatLine& line, State& state, std::function<void(VAddr, T)> write_func,
    Core::System& system) {
...
template <typename T>
static inline typename std::enable_if_t<std::is_integral_v<T>> LoadOp(
    const GatewayCheat::CheatLine& line, State& state, std::function<T(VAddr)> read_func) {
...
static inline void PatchOp(const GatewayCheat::CheatLine& line, State& state, Core::System& system,
    ...
    while (num_bytes >= 4) {
        u32 tmp = first ? cheat_lines[state.current_line_nr].address
                        : cheat_lines[state.current_line_nr].value;
        if (!first && num_bytes != 0) {
...
GatewayCheat::CheatLine::CheatLine(const std::string& line) {
    ...
    try {
        ...
        first = std::stol(line.substr(0, 8), 0, 16);
        address = first & 0x0FFFFFFF;
        value = std::stol(line.substr(10, 8), 0, 16);
        cheat_line = line;
