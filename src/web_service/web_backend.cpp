...
boost::optional<std::unique_ptr<httplib::Client>> GetClientFor(
    ...
    if (parsedUrl.m_Scheme == "http") {
        ...
        cli = std::make_unique<hl::Client>(parsedUrl.m_Host.c_str(), port, TIMEOUT_SECONDS,
                                           hl::HttpVersion::v1_1);
    } else if (parsedUrl.m_Scheme == "https") {
        if (!parsedUrl.GetPort(&port)) {
            port = HTTPS_PORT;
        }
        cli = std::make_unique<hl::SSLClient>(parsedUrl.m_Host.c_str(), port, TIMEOUT_SECONDS,
                                              hl::HttpVersion::v1_1);
    } else {
        LOG_ERROR(WebService, "Bad URL scheme %s", parsedUrl.m_Scheme.c_str());
        return boost::optional<std::unique_ptr<hl::Client>>();
    }

    return boost::optional<std::unique_ptr<hl::Client>>(std::move(cli));
}
...
std::future<Common::WebResult> PostJson(const std::string& url, const std::string& data,
    ...
    return std::async(std::launch::async, [url, parsedUrl, params, data] {
        boost::optional<std::unique_ptr<hl::Client>> cli = GetClientFor(parsedUrl);
